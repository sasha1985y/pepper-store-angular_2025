...
View-first framework* (неофициальная терминология). Основной упор делается на иерархию вью (задается через шаблоны). Все вспомогательные штуки называются Service и подключаются во вью (без контроллера)
...
*Angular скорее всего можно отнести к MVVC, поэтому чистого контроллера у него быть не может
- Model (класс для работы с данными) — Service
- View (шаблон — html-файл)
- ViewController (контроллер, который во главу угла ставит отображение — класс компонента)
